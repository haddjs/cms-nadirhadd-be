...
import { getAllExperiences, getExperienceById, addExperience, updateExperience, deleteExperience } from "../models/experience.model";
...
const getExperiencesController = async (_req: Request, res: Response) => {
    try {
        ...
        console.error("Error in getExperiences controller:", error);
...
const getExperienceByIdController = async (req: Request, res: Response) => {
    try {
        ...
        console.error("Error in getExperienceById controller:", error);
        res.status(500).json({ error: "Internal server error" });
    }
}
...
const addExperienceController = async (req: Request, res: Response) => {
    try {
        ...
        if (!req.body.company_name || !req.body.role || !req.body.start_date) {
            return res.status(400).json({ error: "Missing required fields: company_name, role, start_date" });
        }
...
const updateExperienceController = async (req: Request, res: Response) => {
    try {
        ...
        res.status(200).json({ message: "Experience updated!", data: { id, ...req.body } });
    } catch (error) {
...
export { getExperiencesController, getExperienceByIdController, addExperienceController, updateExperienceController, deleteExperienceController }